...
public:
    ...
    ErrorDetails decode(const String &response, uint8_t errorCode)
    {
        ...
        if (!Utils::validateASCPacketStructure(response, WBusCommandBuilder::CMD_READ_ERRORS, WBusCommandBuilder::ERROR_READ_DETAILS, 16))
        {
            Serial.println("Ошибка валидации пакета");
            return details;
        }
