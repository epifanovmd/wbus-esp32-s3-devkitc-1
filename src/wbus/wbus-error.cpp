...
void WebastoError::hexStringToBytes(const String &hexString, uint8_t *output, int maxLength)
{
...
String WebastoError::decodeErrorPacket(const String &packet)
{
...
String WebastoError::decodeErrorList(const uint8_t *data, uint8_t dataLength)
{
    String result = "";
...
String WebastoError::decodeSingleError(uint8_t errorCode, uint8_t counter)
{
    String result = "0x";
    if (errorCode < 0x10)
        result += "0";
...
void WebastoError::check()
{
    wbusQueue.add(CMD_READ_ERRORS_LIST,
                  [this](bool status, String tx, String rx)
                  {
                      if (!status)
                      {
                          Serial.println("❌ Ошибка чтения ошибок Webasto");
                          return;
                      }

                      String result = this->decodeErrorPacket(rx);
                      Serial.println();
